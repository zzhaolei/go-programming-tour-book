...
import (
	...
	"github.com/zzhaolei/go-programming-tour-book/tour/internal/timer"
)
...
var calculateTimeCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if calculateTime == "" {
			...
			currentTimer, err = timer.ParseInLocation(layout, calculateTime)
			if err != nil {
				t, _ := strconv.Atoi(calculateTime)
				currentTimer = time.Unix(int64(t), 0).In(timer.Location)
			}
		}
